...
void Voronoi_float(float3 normal, float3 pos, float AngleOffset, float CellDensity, out float Out, out float Cells) 
{
    float2 UV = (0, 0);
    if (abs(dot(normal, float3(1, 0, 0))) >= 0.8)
        UV = pos.yz;
    else if (abs(dot(normal, float3(0, 0, 1))) >= 0.8)
        UV = pos.xy;
    else
        UV = pos.xz;
